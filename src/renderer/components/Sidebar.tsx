      ...
      <div className={`no-drag mb-7 flex items-center ${collapsed ? "justify-center" : "gap-3"}`}>
        ...
        {!collapsed && (
          <div className="min-w-0 flex-1">
            <div className="text-lg font-semibold tracking-normal text-[#e8eaed]">Mirrow</div>
            <div className="text-xs text-[#9aa0a6]">AI Web Translator</div>
          </div>
        )}
        {!collapsed && (
          <button
      ...
      {!collapsed && (
        <div className="no-drag mt-6 flex min-h-0 flex-1 flex-col">
          ...
          <div className="mb-3 flex items-start justify-between gap-3">
            <div>
              <div className="text-xs uppercase text-[#9aa0a6]">Engine</div>
              <div className="mt-1 break-words text-sm font-semibold text-[#e8eaed]">{engineModel}</div>
              <div className="mt-1 text-xs text-[#9aa0a6]">{engineProvider}</div>
            </div>
            <MonitorCog className="text-[#8ab4f8]" size={19} />
          </div>
          <div className="flex items-center justify-between">
            <StatusBadge connected={status.data?.connected} loading={status.isLoading} />
            <Link to="/settings" className="text-xs font-medium text-[#8ab4f8] hover:text-[#aecbfa]">
              Change
            </Link>
          </div>
          <div className={`mt-3 grid gap-1 rounded-full bg-[#202124] p-1 text-xs ${enabledEngines.length >= 4 ? "grid-cols-4" : enabledEngines.length === 3 ? "grid-cols-3" : enabledEngines.length === 2 ? "grid-cols-2" : "grid-cols-1"}`}>
            {enabledEngines.map((engine) => (
              <button
                key={engine}
                className={`rounded-full px-2 py-1.5 transition ${selectedEngine === engine ? "bg-[#8ab4f8] text-[#202124]" : "text-[#bdc1c6] hover:bg-white/[0.08]"}`}
          ...
          {selectedEngine === "local" ? (
            <div className="mt-3 rounded-lg border border-[#5f6368]/60 bg-[#202124] px-2 py-2">
              <div className="text-xs uppercase text-[#9aa0a6]">URL</div>
              <div className="mt-1 truncate text-xs text-[#e8eaed]">{settings.data?.lmStudioBaseUrl}</div>
            </div>
          ) : (
            <label className="mt-3 block">
              <span className="mb-1.5 block text-xs uppercase text-[#9aa0a6]">Model</span>
              <div className="relative">
                <select
                  className="h-9 w-full appearance-none rounded-lg border border-[#5f6368]/60 bg-[#202124] px-2 pr-10 text-xs text-[#e8eaed] outline-none transition focus:border-[#8ab4f8]"
                  value={engineModel}
                ...
                <ChevronDown className="pointer-events-none absolute right-4 top-1/2 -translate-y-1/2 text-[#bdc1c6]" size={16} />
              </div>
            </label>
          )}
          {(selectedEngine === "online" || selectedEngine === "google" || selectedEngine === "openrouter") && (
            <div className="mt-3 rounded-lg border border-[#5f6368]/60 bg-[#202124] p-3">
              <div className="flex items-center justify-between">
                <span className="text-xs uppercase text-[#9aa0a6]">
              ...
              <div className="mt-1 text-lg font-semibold text-[#e8eaed]">{Math.round(onlineCost).toLocaleString("en-US")} Toman</div>
            </div>
          )}
            </section>
